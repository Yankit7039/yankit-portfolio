...
const XConfig = {
  count: 200,
  colors: [0xffffff, 0x1e293b, 0x0f172a, 0x000000, 0x3b82f6],
...
const Ballpit: React.FC<BallpitProps> = ({
  ...
  useEffect(() => {
    ...
    // Responsive ball count based on screen size
    const getResponsiveCount = () => {
      if (typeof window !== 'undefined') {
        if (window.innerWidth < 768) { // Mobile
          return Math.floor((props.count || 20) * 0.4); // 40% of original count
        } else if (window.innerWidth < 1024) { // Tablet
          return Math.floor((props.count || 20) * 0.7); // 70% of original count
        }
      }
      return props.count || 20; // Desktop - full count
    };
    ...
    // Handle resize to update ball count
    const handleResize = () => {
      if (spheresInstanceRef.current) {
        const newCount = getResponsiveCount();
